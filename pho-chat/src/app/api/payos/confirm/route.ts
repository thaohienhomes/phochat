...
export async function POST(req: NextRequest) {
  try {
    ...
    const queryWebhookUrl = req.nextUrl.searchParams.get("webhookUrl") || undefined;
    const webhookUrl = (body?.webhookUrl as string | undefined) ?? queryWebhookUrl;

    if (!webhookUrl) {
      logger.error("Confirm missing webhookUrl", { contentType, raw: raw?.slice(0, 200) });
    ...
    logger.error("Failed to confirm PayOS webhook URL", { error: e?.message || String(e) });
    return new Response(
...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error("Failed to confirm PayOS webhook URL (GET)", { error: e?.message || String(e) });
    return new Response(
      JSON.stringify({ error: e?.message || "Failed to confirm webhook" }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
